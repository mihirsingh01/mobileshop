...
def add_product_view(request):
    ...
    return render(request, 'repairing_staff/add_product.html')

@login_required
def product_list_view(request):
    products = Product.objects.all()
    return render(request, 'repairing_staff/product_list.html', {'products': products})
...
def add_sale_view(request):
    ...
    products = Product.objects.all()
    return render(request, 'repairing_staff/add_sale.html', {'products': products})

@login_required
def sales_list_view(request):
    sales = Sale.objects.all()
    return render(request, 'repairing_staff/sales_list.html', {'sales': sales})

@api_view(['GET'])
def api_list(request):
    # Assuming the model is RepairingStaff, but from the views, it's Attendance, Product, Sale
    # For simplicity, return a list of products or sales. Let's use Product as example.
